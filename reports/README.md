...
## 論点B検証: `rr_target` 感度分析(2026-06-20、不採用)

spec.md §11 論点Bの「`rr_target` 既定3.0を2.5/3.0/5.0で比較最適化する」を検証した
(`be_mode=off` 既定構成、XAUUSD M30・5年・スワップ込み)。

### 粗いグリッド(2.0〜5.0)

| rr_target | トレード数 | PF | 勝率 | 最大DD | 純益(5年) |
|---|---|---|---|---|---|
| 2.0 | 272 | 0.972 | 33.09% | $1,140.92 | **−$298.22**(赤字) |
| 2.5 | 208 | 1.440 | 33.17% | $1,166.36 | +$3,937.44 |
| **3.0**(既定) | 197 | **1.456** | 31.98% | $911.78 | +$4,258.48 |
| 4.0 | 153 | 1.651 | 29.41% | $1,135.04 | +$4,818.60 |
| 5.0 | 131 | 1.445 | 20.61% | $1,060.28 | +$2,985.34 |

一見 `rr=4.0` がPF・純益ともに最良に見えたため、`3.25〜4.5` で細かく再探索した:

### 細かいグリッド(3.25〜4.5、`rr=4.0` 周辺の再探索)

| rr_target | トレード数 | PF | 勝率 | 最大DD | 純益(5年) |
|---|---|---|---|---|---|
| 3.25 | 166 | 1.549 | 29.52% | $1,019.60 | +$4,197.68 |
| 3.5 | 174 | 1.352 | 25.86% | $1,102.42 | +$3,201.80 |
| 3.75 | 128 | **1.867** | 28.13% | $884.76 | **+$5,440.24** |
| 4.0 | 153 | 1.651 | 29.41% | $1,135.04 | +$4,818.60 |
| 4.25 | 127 | 1.372 | 25.98% | $1,135.04 | +$2,594.96 |
| 4.5 | 134 | 1.489 | 23.88% | $943.22 | +$3,158.66 |

**結果: トレード数・PF・純益のいずれも `rr_target` に対して非単調かつ激しく振動し、滑らかな
最適化曲面になっていない。** `rr=3.5`(PF1.352)は隣接する `rr=3.25`(PF1.549)・`rr=4.0`
(PF1.651)よりも明確に劣り、`rr=3.75` は単発でPF1.87という最良値を出すが両隣(`3.5`・`4.0`)
は3.0既定より劣化する。これは典型的な**過剰適合(オーバーフィット)のシグナル**であり、
「`rr=3.75` がたまたまこの5年間の価格経路に合致した」だけの可能性が高い。

**構造的な理由**: `rr_target` はTP距離を通じて、本手法の単一ポジション制約(自己ミラー追跡。
spec.md §2.5)の「建玉中とみなす期間」の長さを変える。建玉期間が変わると、その間に発生した
はずの別のBOSシグナルが抑制されるか否かが切り替わるため、`rr_target` のわずかな変化が
**どのトレードが成立するか自体**を非連続に変える(出口の質が変わるだけではない)。これが
滑らかな最適化を妨げている。

**判定: `rr_target=3.0`(既定)を変更しない。** ノイズの多い単発の最良値(`rr=3.75`)を
「最適化」として採用するのは過剰適合であり、本プロジェクトが他の実験(`depth50` vs
`depthtier2` 等)で一貫して回避してきた判断基準と矛盾する。`rr=2.0` が明確な赤字
(PF0.97)である点のみが頑健な知見(=過度に低いRRは不採用)。spec.md §11 論点Bはこの
結論で解決済みとする。
