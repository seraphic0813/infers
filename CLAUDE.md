...
規約は「**全手法・プラットフォーム共通の不変条件(A)**」と「**Narrow Focus 手法固有の契約(B)**」に分かれる。A は L0/L1 とすべての手法が必ず守る。B は Narrow Focus 手法だけの定義であり、別ロジックの手法(例: `market_tpsl`)は B の一部(コンフルエンス・半分利確・エリオット等)を持たなくてよい — ただし A は全手法が無条件に守る。
...
4. **防御調整に含み益トリガー禁止** — SL移動・建値化等の防御調整を PnL・pips・含み益を条件に行わない。トリガーは価格構造イベントに限る(Narrow Focus での具体機構は B を参照)。
...
- **建値SL移動はダウ状態機械のイベント駆動** — 「安値切り上げ/高値切り下げ確定」イベントでのみ建値化(設計書 §6.3)。PnLやpipsを条件に使わない(A-4 の具体機構)。
