"""Narrow Focus 執行状態機械 (L2 / 設計書 §6.1〜6.4 / CLAUDE.md 第1〜5・9〜10条)。

打診→追撃→建値SL→半分利確→ランナーという Narrow Focus 固有の執行手順を持つため
手法側 (L2) に属する。汎用の安全境界 (BrokerPort・SL単調性ガード・FSM エラー) と
ExecutionModel 抽象は L0 (`core/execution.py`) から import する。旧パス
`infers.execution.sm` は互換 re-export で維持される (段階2.3b)。
...
from infers.core.execution import (
    BarOutcome, BrokerPort, SlMonotonicityError, TransitionError,
)
