...
> **既知の制約と修正(2026-06-22、ライブ検証で発見)**: 自己ミラー式の単一ポジション制約
> (§2.5・`provider.py` の `_position`)は、`LiveRunner`/ダッシュボードのウォームアップ
> (歴史データを `on_candle` へ素通しして指標だけ育てる経路。発注しない)でも素通しされる
> ため、ウォームアップ窓内に本物のBOSシグナルが含まれていると、一度も発注していないのに
> ミラーだけ「建玉中」になり、以後ずっと新規プランがブロックされ続ける事故が実際に発生した
> (2026-06-21〜のライブ検証。ウォームアップ直前の2026-06-20 00:30 UTCの実シグナルが原因)。
> **修正**: `SmcBosProvider.reset_position_mirror()` を新設し、`LiveRunner._warm_provider()`
> および `dashboard._WarmupProvider.warmup()` がウォームアップ完了直後に(手法固有の語彙を
> 持たない汎用フックとして)呼び出すようにした。回帰テストは `tests/test_smc_bos.py
> ::test_reset_position_mirror_clears_phantom_position` 等。再起動時に実ブローカー側へ
> 本当に建玉が残っているケース(プロセスが建玉中にクラッシュした場合)は、本修正の対象外
> (`reconcile()` はFSM/ジャーナル側の整合は取るが、プロバイダのミラーへは現状反映されない。
> 既知の制約として残置)。
