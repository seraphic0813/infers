"""smc_bos 執行モデル: 成行参入 + 固定SL/RR利確 (L2 / 段階S2 / spec.md §3.4)。

`core.execution.ExecutionModel` を構造的に充足する執行モデル。段階S2は
`be_mode=off`(spec.md §3.3)の最小ライフサイクルのみを実装する:
...
SL前進(`be_mode=at_1r`/`structure`)は段階S4で追加する。本モジュールは
段階S2のスコープに限定し、未実装のSL前進ロジックを持たない。

安全原則 (CLAUDE.md §A) は本モデルにも等しく強制される:
  - A-1 LLM非依存 — 本モジュールは LLM を一切呼ばない (防御は決定論)。
  - A-2 確定足主義 — on_bar は確定足のみ受領する。
  - A-3 SL単調性 — 固定SLは一度も動かさない (move_sl 経路を持たない) ため自明に満たす。
...
class SmcExecution:
    """成行参入 + 固定SL/RR利確の執行モデル (段階S2。ExecutionModel を構造的に充足)。"""
