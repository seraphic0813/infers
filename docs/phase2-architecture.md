...
| **2.3b** | **L0↔L2 境界違反の解消**。`TradePlan`/`ProviderOutput` を L2 `strategies/narrow_focus/signals.py` へ、`NarrowFocusExecution`/`PosState`/`FsmConfig` を L2 `strategies/narrow_focus/execution.py` へ物理移設(`git mv`)。汎用 `BrokerPort`/`SlMonotonicityError`/`TransitionError` は L0 `core/execution.py` へ。`core/loop.py`・`execution/sm.py`・`execution/__init__.py` から `analysis` 系手法固有型の import を完全除去(grep で確認)。旧パス `infers.core.loop`/`infers.execution.sm`/`infers.execution` は PEP 562 `__getattr__` 遅延 re-export で互換維持(L0がL2をモジュールレベル import しない) | 中〜高 | テスト324件合格 + L0純度 grep 確認 + depth50数値完全一致 | **完了** (2026-06-20) |
...
| **2.5** | **2つ目の手法 `market_tpsl`** を追加し抽象を検証。`strategies/market_tpsl/`(`execution.py`: 成行参入+固定TP/SL の `MarketTpSlExecution`=IDLE→OPEN→CLOSED の別ライフサイクル、`provider.py`: SMAクロスの `SmaCrossProvider`)。`StrategySpec.build_execution` を追加し手法ごとに執行モデルを差替可能化。`BacktestEngine`→`TradingLoop` へ `execution_factory` を配線(None=既定の Narrow Focus 執行) | 中 | テスト331件合格(新規7: 執行単体4・レジストリ配線2・engine結合1)+ depth50ビット完全一致(`execution_factory=None` 経路不変)+ `--strategy market_tpsl` CLI が完走(6か月で792クロス検出) | **完了** (2026-06-20) |
...
### 段階2.5で判明した残課題: AIゲートの手法結合 (フェーズ3候補)

`TradingLoop` の新規プラン処理は `gateway.judge(plan.request, ...)` を**全手法に強制**する。
このゲート(ルール判定 `rule_judge.py` / verdict_cache)は Narrow Focus 固有の特徴量を前提に
しており、別手法(`market_tpsl`)のプランは特徴量不一致で GUARDRAIL NO_GO になる
(`--strategy market_tpsl` の CLI バックテストは 6か月で 792 クロスを検出するが全件 NO_GO で
0トレード)。したがって 2.5 の抽象検証は**寛容ゲート(常時GO)を注入した結合テスト**で行い、
`MarketTpSlExecution` が BacktestEngine→TradingLoop 経由でエントリー→TP決済まで成立することを
確認した。AIゲートを「手法に属する任意ステップ」へ一般化(Strategy 側がゲートの要否・特徴量を
決める)するのはフェーズ3の課題。なお防御層(SL単調性・冪等ID・リスク拒否権)は全執行モデルに
等しく強制され続けており、この結合は安全性ではなく「別手法が約定できるか」だけの問題。

### 段階2.0で判明した既知の境界違反 (2.3bで解消済み)

(当初の問題) `core/loop.py` と `execution/sm.py`(L0)が `analysis.dow.StructureEvent` と
`analysis.support_resistance.SRZone`(本来はNarrow Focus固有のL2型)を直接 import していた。

**解消済み (2.3a→2.3b)**:
- 2.3a で `TradingLoop` のシグナル型を `object` へ一般化(`on_bar(candle, signal)`)。
- 2.3b で `TradePlan`/`ProviderOutput` を `strategies/narrow_focus/signals.py` へ、
  執行 FSM 本体(`NarrowFocusExecution`/`PosState`/`FsmConfig`)を
  `strategies/narrow_focus/execution.py` へ `git mv` 移設。汎用の `BrokerPort`/
  `SlMonotonicityError`/`TransitionError` は `core/execution.py`(L0)へ集約。
- 結果、`core/loop.py`・`execution/sm.py`・`execution/__init__.py` から `analysis` 系
  手法固有型のモジュールレベル import が消えた(`grep "from infers.analysis"` が空)。
- 旧 import パス(`infers.core.loop`・`infers.execution.sm`・`infers.execution`)は
  PEP 562 モジュール `__getattr__` による遅延 re-export で互換維持。これにより
  L0 のシム自身が L2 をモジュールレベルで import することもない。
- 残課題: `analysis/dow.py` が `strategies.narrow_focus.zigzag.SwingPoint` を参照する
  件は、`analysis/` 配下のダウ判定ロジック自体を L2 手法側へ移すか否かの別論点
  (2.5 で2つ目の手法を入れる際に、ダウ理論が手法共通部品か Narrow Focus 固有かを判断)。
